...
import {
  notifyPrivateRoomMessage,
  notifyPrivateRoomModeration
...
export async function removeMemberFromPrivateRoom({
  ...
  await notifyPrivateRoomModeration({
  room,
...
export async function setPrivateRoomMemberMute({
  ...
  await requirePrivateRoomMember(
  roomId,
  targetUserId
);

const updatedMember =
  await updatePrivateRoomMemberMuteStatus(
    roomId,
    targetUserId,
    Boolean(
      isMuted
...
export async function sendPrivateRoomMessage({
  ...
  await updatePrivateRoomLastActivity(
  room.room_id
);

const completeMessage =
  await findPrivateRoomMessageById(
    message.chat_message_id
  );

await notifyPrivateRoomMessage({
  room,
...
export async function deletePrivateRoomMessage({
  ...
  const roomOwner =
    await isPrivateRoomOwner(
      message.room_id,
      userId
    );

  /*
   * Explicitly calculate permissions.
   */
  const isSender =
   ...
   * IMPORTANT:
   *
   * chat_messages.deleted_by is NOT
   * intended to store the user's UUID.
   *
   * Existing chat code already uses
   * "sender", while system/room cleanup
   * uses "system".
   */
  const deletedBy =
    isSender
